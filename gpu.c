...
void copia_vetor(float *dest, float *orig, int tdom) {
    ...
    #pragma acc parallel loop //present(dest[0:tdom], orig[0:tdom])
    for (i = 0; i < tdom; i++) {
        dest[i] = orig[i];
    }
}

void mult_mat_vet(float **matriz, int *offset, float *vetor, float *result,
                  int tdom) {
    float tmp;
    int i, j;

    #pragma acc parallel loop //private(tmp) present(matriz[0:tdom][0:5], offset[0:5], vetor[0:tdom], result[0:tdom])
    for (i = 0; i < tdom; i++) {
        tmp = 0.00;
        for (j = 0; j < 5; j++) {
...
float produto_escalar(float *vetor1, float *vetor2, int tdom) {
    ...
    #pragma acc parallel loop reduction(+:resposta) //present(vetor1[0:tdom], vetor2[0:tdom])
    for (i = 0; i < tdom; i++)
...
void escalar_vetor(float *vetor, float escalar, float *resposta, int tdom) {
    ...
    #pragma acc parallel loop //present(vetor[0:tdom], resposta[0:tdom])
    for (i = 0; i < tdom; i++)
...
void soma_vetor(float *vetor1, float *vetor2, float *resposta, int tdom) {
    ...
    #pragma acc parallel loop //present(vetor1[0:tdom], vetor2[0:tdom], resposta[0:tdom])
    for (i = 0; i < tdom; i++)
...
//              Gradiente Conjugado                 //
//////////////////////////////////////////////////////
void GC(float **matriz, int *offset, float *vet1, float *resp, int tdom) {
    int it;
...
//--------------------------------------------------------
int geramatriz(float **dominio, float **matA, float *vetb, float *vetb_ext,
               int m, int n) {
    int k, i, j;
    float C, X;
